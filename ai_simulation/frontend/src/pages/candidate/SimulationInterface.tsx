...
export const SimulationInterface: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) navigate('/candidate/dashboard');
      } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate(`/simulation/${attemptId}/complete`);
  ...
  if (loading || !blueprint) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-4">
      ...
      {!loading && !blueprint && (
        <button onClick={() => navigate('/candidate/dashboard')} className="mt-2 text-indigo-600 text-sm hover:text-indigo-500">
          ← Back to Dashboard
        </button>
      )}
    </div>
