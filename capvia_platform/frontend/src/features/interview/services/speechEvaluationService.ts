...
function keywords(q: string): string[] {
  return meaningful(q).filter((val, index, self) => self.indexOf(val) === index);
}
...
export function evaluateAll(answers: AnswerRecord[]): EvaluationReport {
  ...
  const allMiss = results.flatMap(r => r.missingKeywords).filter((val, index, self) => self.indexOf(val) === index).slice(0, 4);
